/*ILLUMINATION_BUF*/
        ...
        s1.x = native_divide(((float)volData[id]), 255.0f);

        id = clamp((iCoords.x + 1u) + iCoords.y * volRes.x + iCoords.z * volRes.x * volRes.y, 0u, numVoxels - 1u);
        s2.x = native_divide(((float)volData[id]), 255.0f);

        id = clamp(iCoords.x + (iCoords.y - 1u) * volRes.x + iCoords.z * volRes.x * volRes.y, 0u, numVoxels - 1u);
        s1.y = native_divide(((float)volData[id]), 255.0f);

        id = clamp(iCoords.x + (iCoords.y + 1u) * volRes.x + iCoords.z * volRes.x * volRes.y, 0u, numVoxels - 1u);
        s2.y = native_divide(((float)volData[id]), 255.0f);

        id = clamp(iCoords.x + iCoords.y * volRes.x + (iCoords.z - 1u) * volRes.x * volRes.y, 0u, numVoxels - 1u);
        s1.z = native_divide(((float)volData[id]), 255.0f);

        id = clamp(iCoords.x + iCoords.y * volRes.x + (iCoords.z + 1u) * volRes.x * volRes.y, 0u, numVoxels - 1u);
        s2.z = native_divide(((float)volData[id]), 255.0f);
