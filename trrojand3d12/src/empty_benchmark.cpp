...
//
//    const auto cpu_iterations = config.get<std::uint32_t>(factor_cpu_counter_iterations);
//    timer cpu_timer;
//
//    auto gpu_freq = gpu_timer::get_timestamp_frequency(device.command_queue());
//    const auto gpu_iterations = config.get<std::uint32_t>(factor_gpu_counter_iterations);
//    gpu_timer gpu_timer(device.d3d_device(), 2, this->pipeline_depth());
//    std::vector<gpu_timer::millis_type> clear_times(gpu_iterations), gpu_times(gpu_iterations);
//
